...
using ov::genai::Qwen3ForcedAligner;
...
TEST(Qwen3ForcedAlignerTokenize, UnicodeWhitespaceSplitsWords) {
    // Match Python str.split() whitespace semantics used by the reference processor.
...
TEST(Qwen3ForcedAlignerSegment, UnimplementedLanguageStrategiesRejected) {
    ...
    EXPECT_ANY_THROW(segment_alignment_units("text", "arabic"));
}
...
TEST(Qwen3ForcedAlignerLanguage, NormalizationAliases) {
    ...
    EXPECT_EQ(normalize_alignment_language("Cantonese"), "cantonese");
    EXPECT_EQ(normalize_alignment_language("Russian"), "russian");
    EXPECT_EQ(normalize_alignment_language("Portuguese"), "portuguese");
    ...
    EXPECT_EQ(normalize_alignment_language("Arabic"), "arabic");
...
// Real-model smoke test enabled with QWEN3_FORCED_ALIGNER_MODEL.
TEST(Qwen3ForcedAlignerComponent, AlignsMonotonicWithinChunk) {
    ...
    Qwen3ForcedAligner aligner{model_dir, "CPU", {}};

    std::vector<float> silence(16000, 0.0f);
    const auto words = aligner.align(silence, "how are you", "English", 0.0f);
    ASSERT_EQ(words.size(), 3u);
    float previous = -1.0f;
    for (const auto& word : words) {
        EXPECT_LE(word.start_ts, word.end_ts);
        EXPECT_GE(word.start_ts, previous);
        EXPECT_TRUE(word.token_ids.empty());
        previous = word.start_ts;
    }
}
