import BasicGenerationConfiguration from '@site/docs/use-cases/_shared/_basic_generation_configuration.mdx';
import BeamSearchGeneration from '@site/docs/use-cases/_shared/_beam_search_generation.mdx';
import GenerationConfigurationWorkflow from '@site/docs/use-cases/_shared/_generation_configuration_workflow.mdx';
import Streaming from '@site/docs/use-cases/_shared/_streaming.mdx';

## Additional Usage Options

:::tip
Check out [Python](https://github.com/openvinotoolkit/openvino.genai/tree/master/samples/python/automatic_speech_recognition), [C++](https://github.com/openvinotoolkit/openvino.genai/tree/master/samples/cpp/automatic_speech_recognition), and [JavaScript](https://github.com/openvinotoolkit/openvino.genai/tree/master/samples/js/automatic_speech_recognition) speech recognition samples.
:::

### Use Different Generation Parameters

<GenerationConfigurationWorkflow />

<BasicGenerationConfiguration>
  <LanguageTabs>
      <TabItemPython>
          ```python
          import openvino_genai as ov_genai

          pipe = ov_genai.ASRPipeline(model_path, "CPU")

          # Get default configuration
          config = pipe.get_generation_config()

          # Modify parameters
          config.max_new_tokens = 100
          config.temperature = 0.7
          config.top_k = 50
          config.top_p = 0.9
          config.repetition_penalty = 1.2

          # Generate text with custom configuration
          result = pipe.generate(raw_speech, config)
          ```
      </TabItemPython>
      <TabItemCpp>
          ```cpp
          int main() {
              ov::genai::ASRPipeline pipe(model_path, "CPU");

              // Get default configuration
              auto config = pipe.get_generation_config();

              // Modify parameters
              config.max_new_tokens = 100;
              config.temperature = 0.7f;
              config.top_k = 50;
              config.top_p = 0.9f;
              config.repetition_penalty = 1.2f;

              // Generate text with custom configuration
              auto result = pipe.generate(raw_speech, config);
          }
          ```
      </TabItemCpp>
      <TabItemJS>
          ```javascript
          import { ASRPipeline } from 'openvino-genai-node';

          const pipeline = await ASRPipeline(modelPath, "CPU");

          // Get default configuration
          const config = pipeline.getGenerationConfig();

          // Modify parameters
          const generationConfig = {
            ...config,
            max_new_tokens: 100,
            temperature: 0.7,
            top_k: 50,
            top_p: 0.9,
            repetition_penalty: 1.2
          };

          // Generate text with custom configuration
          const result = await pipeline.generate(rawSpeech, { generationConfig });
          ```
      </TabItemJS>
  </LanguageTabs>
</BasicGenerationConfiguration>

<BeamSearchGeneration>
  <LanguageTabs>
      <TabItemPython>
          ```python
          import openvino_genai as ov_genai

          pipe = ov_genai.ASRPipeline(model_path, "CPU")

          # Get default generation config
          config = pipe.get_generation_config()

          # Modify parameters
          config.max_new_tokens = 256
          config.num_beams = 15
          config.num_beam_groups = 3
          config.diversity_penalty = 1.0

          # Generate text with custom configuration
          result = pipe.generate(raw_speech, config)
          ```
      </TabItemPython>
      <TabItemCpp>
          ```cpp
          int main() {
              ov::genai::ASRPipeline pipe(model_path, "CPU");

              // Get default generation config
              ov::genai::ASRGenerationConfig config = pipe.get_generation_config();

              // Modify parameters
              config.max_new_tokens = 256;
              config.num_beams = 15;
              config.num_beam_groups = 3;
              config.diversity_penalty = 1.0f;

              // Generate text with custom configuration
              auto result = pipe.generate(raw_speech, config);
          }
          ```
      </TabItemCpp>
      <TabItemJS>
          ```javascript
          const pipeline = await ASRPipeline(modelPath, "CPU");

          // Get default generation config
          const config = pipeline.getGenerationConfig();

          // Modify parameters
          const generationConfig = {
            ...config,
            max_new_tokens: 256,
            num_beams: 15,
            num_beam_groups: 3,
            diversity_penalty: 1.0
          };

          // Generate text with custom configuration
          const result = await pipeline.generate(rawSpeech, { generationConfig });
          ```
      </TabItemJS>
  </LanguageTabs>
</BeamSearchGeneration>

:::note
Beam search support depends on the selected model architecture. Qwen3-ASR models do not support beam search decoding.
:::

### Transcription

Language handling depends on the model architecture. Models with language identification support can automatically detect the language of the input audio. You can also specify the language to improve accuracy. The detected or specified language is available in the result via the `languages` field.

<LanguageTabs>
    <TabItemPython>
        ```python
        pipe = ov_genai.ASRPipeline(model_path, "CPU")

        # Automatic language detection for models with language identification support
        raw_speech = read_wav("speech_sample.wav")
        result = pipe.generate(raw_speech)
        print(result.languages[0])  # e.g. "en", "fr", "de"

        # Explicitly specify language. In the form of `en`, `<|en|>` for Whisper and `English` for Qwen3-ASR
        result = pipe.generate(raw_speech, language="en")
        print(result.languages[0])  # "en"

        # French speech sample
        raw_speech = read_wav("french_sample.wav")
        result = pipe.generate(raw_speech, language="<|fr|>")
        print(result.languages[0])  # "fr"
        ```
    </TabItemPython>
    <TabItemCpp>
        ```cpp
        int main() {
            ov::genai::ASRPipeline pipe(model_path, "CPU");

            // Automatic language detection for models with language identification support
            auto result = pipe.generate(raw_speech);
            std::cout << result.languages[0] << "\n";  // e.g. "en", "fr", "de"

            // Explicitly specify language. In the form of `en`, `<|en|>` for Whisper and `English` for Qwen3-ASR
            result = pipe.generate(raw_speech, ov::genai::language("<|en|>"));
            std::cout << result.languages[0] << "\n";  // "en"

            // French speech sample
            raw_speech = utils::audio::read_wav("french_sample.wav");
            result = pipe.generate(raw_speech, ov::genai::language("<|fr|>"));
            std::cout << result.languages[0] << "\n";  // "fr"
        }
        ```
    </TabItemCpp>
    <TabItemJS>
        ```javascript
        const pipeline = await ASRPipeline(modelPath, "CPU");

        // Automatic language detection for models with language identification support
        let result = await pipeline.generate(rawSpeech);

        // Explicitly specify language (English)
        let generationConfig = { language: "<|en|>" };
        result = await pipeline.generate(rawSpeech, { generationConfig });

        // French speech sample
        generationConfig = { language: "<|fr|>" };
        result = await pipeline.generate(frenchRawSpeech, { generationConfig });
        ```
    </TabItemJS>
</LanguageTabs>

### Whisper-specific Features

:::info
These features are specific to Whisper models used with `ASRPipeline`. They may not apply to other ASR model architectures.
:::

#### Translation

By default, Whisper performs transcription, keeping the output in the same language as the input.
To translate non-English speech to English, use the `translate` task:

<LanguageTabs>
    <TabItemPython>
        ```python
        pipe = ov_genai.ASRPipeline(model_path, "CPU")

        # Translate French audio to English
        raw_speech = read_wav("french_sample.wav")
        result = pipe.generate(raw_speech, task="translate")
        ```
    </TabItemPython>
    <TabItemCpp>
        ```cpp
        int main() {
            ov::genai::ASRPipeline pipe(model_path, "CPU");

            // Translate French audio to English
            raw_speech = utils::audio::read_wav("french_sample.wav");
            result = pipe.generate(raw_speech, ov::genai::task("translate"));
        }
        ```
    </TabItemCpp>
    <TabItemJS>
        ```javascript
        const pipeline = await ASRPipeline(modelPath, "CPU");

        // Translate French audio to English
        const generationConfig = { task: "translate" };
        const result = await pipeline.generate(rawSpeech, { generationConfig });
        ```
    </TabItemJS>
</LanguageTabs>

#### Timestamps Prediction

Whisper can predict timestamps for each segment of speech, which is useful for synchronization or creating subtitles:

<LanguageTabs>
    <TabItemPython>
        ```python
        pipe = ov_genai.ASRPipeline(model_path, "CPU")

        # Enable timestamp prediction
        result = pipe.generate(raw_speech, return_timestamps=True)

        # Print timestamps and text segments
        for chunk in result.chunks[0]:
            print(f"timestamps: [{chunk.start_ts:.2f}, {chunk.end_ts:.2f}] text: {chunk.text}")
        ```
    </TabItemPython>
    <TabItemCpp>
        ```cpp
        int main() {
            ov::genai::ASRPipeline pipe(model_path, "CPU");

            // Enable timestamp prediction
            result = pipe.generate(raw_speech, ov::genai::return_timestamps(true));

            // Print timestamps and text segments
            for (const auto& chunk : (*result.chunks)[0]) {
                std::cout << "timestamps: [" << chunk.start_ts << ", " << chunk.end_ts
                          << "] text: " << chunk.text << "\n";
            }
        }
        ```
    </TabItemCpp>
    <TabItemJS>
        ```javascript
        const pipeline = await ASRPipeline(modelPath, "CPU");

        // Enable timestamp prediction
        const generationConfig = { return_timestamps: true, language: "<|en|>", task: "transcribe" };
        const result = await pipeline.generate(rawSpeech, { generationConfig });

        // Print timestamps and text segments
        for (const chunk of result.chunks?.[0] ?? []) {
          console.log(`timestamps: [${chunk.startTs.toFixed(2)}, ${chunk.endTs.toFixed(2)}] text: ${chunk.text}`);
        }
        ```
    </TabItemJS>
</LanguageTabs>

#### Word-level Timestamps Prediction

Whisper can predict timestamps for each word of speech, which provides more granular timing information compared to segment-level timestamps.

<LanguageTabs>
    <TabItemPython>
        ```python
        # Word timestamps require decomposition of cross-attention decoder SDPA layers,
        # so word_timestamps must be passed to the pipeline constructor (not just in generation config)
        pipe = openvino_genai.ASRPipeline(model_path, "CPU", word_timestamps=True)

        # Enable word-level timestamp prediction
        result = pipe.generate(raw_speech, word_timestamps=True)

        # Print word-level timestamps
        for word in result.words[0]:
            print(f"[{word.start_ts:.2f}, {word.end_ts:.2f}]: {word.text}")
        ```
    </TabItemPython>
    <TabItemCpp>
        ```cpp
        int main() {
            // Word timestamps require decomposition of cross-attention decoder SDPA layers,
            // so word_timestamps must be passed to the pipeline constructor (not just in generation config)
            ov::genai::ASRPipeline pipe(model_path, "CPU", ov::genai::word_timestamps(true));

            // Enable word-level timestamp prediction
            auto result = pipe.generate(raw_speech, ov::genai::word_timestamps(true));

            // Print word-level timestamps
            std::cout << std::fixed << std::setprecision(2);
            for (const auto& word : (*result.words)[0]) {
                std::cout << "[" << word.start_ts << ", " << word.end_ts << "]: " << word.text << "\n";
            }
        }
        ```
    </TabItemCpp>
    <TabItemJS>
        ```javascript
        // Word timestamps require decomposition of cross-attention decoder SDPA layers,
        // so word_timestamps must be passed to the pipeline constructor (not just in generation config)
        const pipeline = await ASRPipeline(modelPath, "CPU", { word_timestamps: true });

        // Enable word-level timestamp prediction
        const generationConfig = { return_timestamps: true, word_timestamps: true, language: "<|en|>", task: "transcribe" };
        const result = await pipeline.generate(rawSpeech, { generationConfig });

        // Print word-level timestamps
        for (const w of result.words?.[0] ?? []) {
          console.log(`[${w.startTs.toFixed(2)}, ${w.endTs.toFixed(2)}]: ${w.text}`);
        }
        ```
    </TabItemJS>
</LanguageTabs>

For Qwen3-ASR word timestamps, OpenVINO GenAI uses the companion Qwen3 Forced Aligner model. Qwen3-ASR and the forced aligner are distributed as separate upstream models. Package the exported forced aligner under a `forced_aligner/` subdirectory of the Qwen3-ASR model directory.

When this component is present, `ASRPipeline` discovers and compiles it automatically at construction using the same device and compile properties as the Qwen3-ASR pipeline. No additional constructor argument is required. Word-level timestamps are then selected with the existing `word_timestamps` generation option.

The `forced_aligner/` component is optional. Qwen3-ASR transcription works normally without it; requesting `word_timestamps=true` without the component raises an error indicating the expected `<model_path>/forced_aligner` location.

OpenVINO GenAI currently supports forced-alignment timestamps for Chinese, Cantonese, English, German, Spanish, French, Italian, Portuguese, and Russian. The Qwen3 Forced Aligner model also supports Japanese and Korean, but their reference segmentation is not yet implemented in OpenVINO GenAI. Forced-aligner word chunks currently have empty `token_ids`.

Because Qwen3-ASR and its forced aligner are distributed separately, prepare the OpenVINO package with two exports. Install `qwen-asr` before exporting:

```sh
pip install qwen-asr

# Export Qwen3-ASR to the package root.
optimum-cli export openvino --model Qwen/Qwen3-ASR-0.6B qwen3-asr-ov

# Export its companion aligner into the expected subdirectory.
optimum-cli export openvino --model Qwen/Qwen3-ForcedAligner-0.6B qwen3-asr-ov/forced_aligner
```

Pass the resulting `qwen3-asr-ov/` directory to `ASRPipeline` as usual:

<LanguageTabs>
    <TabItemPython>
        ```python
        pipe = openvino_genai.ASRPipeline(model_path, "CPU")

        result = pipe.generate(raw_speech, word_timestamps=True)

        for word in result.words[0]:
            print(f"[{word.start_ts:.2f}, {word.end_ts:.2f}]: {word.text}")
        ```
    </TabItemPython>
    <TabItemCpp>
        ```cpp
        int main() {
            ov::genai::ASRPipeline pipe(model_path, "CPU");

            auto result = pipe.generate(raw_speech, ov::genai::word_timestamps(true));

            std::cout << std::fixed << std::setprecision(2);
            for (const auto& word : (*result.words)[0]) {
                std::cout << "[" << word.start_ts << ", " << word.end_ts << "]: " << word.text << "\n";
            }
        }
        ```
    </TabItemCpp>
</LanguageTabs>

#### Long-Form Audio Processing

Whisper models are designed for audio segments up to 30 seconds in length.
For longer audio, the OpenVINO GenAI ASR pipeline automatically handles Whisper processing using a sequential chunking algorithm ("sliding window"):

1. The audio is divided into 30-second segments
2. Each segment is processed sequentially
3. Results are combined to produce the complete transcription

This happens automatically when you input longer audio files.

#### Using Initial Prompts and Hotwords

You can improve transcription quality and guide the model's output style by providing initial prompts or hotwords using the following parameters:

- `initial_prompt`: initial prompt tokens passed as a previous transcription (after `<|startofprev|>` token) to the first processing window.
- `hotwords`: hotwords tokens passed as a previous transcription (after `<|startofprev|>` token) to the all processing windows.

Whisper models can use that context to better understand the speech and maintain a consistent writing style.
However, prompts do not need to be genuine transcripts from prior audio segments.
Such prompts can be used to steer the model to use particular spellings or styles:

<LanguageTabs>
    <TabItemPython>
        ```python
        pipe = ov_genai.ASRPipeline(model_path, "CPU")

        result = pipe.generate(raw_speech)
        # He has gone and gone for good answered Paul Icrom who...

        result = pipe.generate(raw_speech, initial_prompt="Polychrome")
        # He has gone and gone for good answered Polychrome who...
        ```
    </TabItemPython>
    <TabItemCpp>
        ```cpp
        int main() {
            ov::genai::ASRPipeline pipe(model_path, "CPU");

            auto result = pipe.generate(raw_speech);
            // He has gone and gone for good answered Paul Icrom who...

            result = pipe.generate(raw_speech, ov::genai::initial_prompt("Polychrome"));
            // He has gone and gone for good answered Polychrome who...
        }
        ```
    </TabItemCpp>
    <TabItemJS>
        ```javascript
        const pipeline = await ASRPipeline(modelPath, "CPU");

        let result = await pipeline.generate(rawSpeech);
        // He has gone and gone for good answered Paul Icrom who...

        const generationConfig = { initial_prompt: "Polychrome" };
        result = await pipeline.generate(rawSpeech, { generationConfig });
        // He has gone and gone for good answered Polychrome who...
        ```
    </TabItemJS>
</LanguageTabs>

:::info
For the ASR API, refer to the [ASR Generation Config API](https://docs.openvino.ai/2026/api/genai_api/_autosummary/openvino_genai.ASRGenerationConfig.html).
:::

### Performance Metrics

`ASRPipeline` returns `ASRDecodedResults` with a `perf_metrics` field of type `ASRPerfMetrics`, which extends [`PerfMetrics`](/docs/guides/performance-metrics).

In addition to common metrics (TTFT, TPOT, throughput, etc.), `ASRPerfMetrics` provides:

| Metric | Type | Description |
|--------|------|-------------|
| `get_features_extraction_duration()` | `MeanStdPair`, ms | Time spent extracting audio features (mel spectrogram) |
| `get_encode_inference_duration()` | `MeanStdPair`, ms | Encoder model inference duration |
| `get_decode_inference_duration()` | `MeanStdPair`, ms | Decoder model inference duration (all token generation steps) |
| `get_word_level_timestamps_processing_duration()` | `MeanStdPair`, ms | Time spent processing word-level timestamps |

Raw values are available in the `asr_raw_metrics` field of `ASRPerfMetrics`.

<Streaming />
