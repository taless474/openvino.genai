...
constexpr std::string_view FORCED_ALIGNER_SUBDIR = "forced_aligner";
...
Qwen3ASR::Qwen3ASR(const std::filesystem::path& models_path, const std::string& device, const ov::AnyMap& properties)
    ...
    const std::filesystem::path forced_aligner_path = models_path / FORCED_ALIGNER_SUBDIR;
    if (std::filesystem::exists(forced_aligner_path)) {
...
ASRDecodedResults Qwen3ASR::generate(const AudioInputs& audio_inputs,
    ...
    OPENVINO_ASSERT(!config.word_timestamps || m_forced_aligner,
                    "Qwen3-ASR word timestamps require Forced Aligner model assets under '<model_path>/",
                    FORCED_ALIGNER_SUBDIR, "'.");
...
void Qwen3ASR::align_words(const std::vector<AudioChunk>& chunks,
    ...
    for (size_t i = 0; i < chunks.size(); ++i) {
        ...
        std::vector<ASRDecodedResultChunk> chunk_words =
            m_forced_aligner->align(chunks[i].wav, text, language, chunks[i].offset_sec);
