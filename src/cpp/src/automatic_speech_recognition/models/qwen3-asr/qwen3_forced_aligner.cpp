...
constexpr size_t QWEN3_FORCED_ALIGNER_CLASSIFY_NUM = 5000;
...
// Covers the Unicode letter/number ranges needed by the currently supported alignment languages.
...
// Match Python str.split()/str.isspace() whitespace semantics used by the reference preprocessing.
...
std::string normalize_alignment_language(const std::string& language) {
    // Canonicalize Qwen3-ASR language names and forced-aligner language-code aliases.
    ...
    const auto it = alias_to_canonical.find(to_lower(language));
...
Qwen3ForcedAligner::Qwen3ForcedAligner(const std::filesystem::path& models_path,
    ...
    size_t classify_num = 0;
    ov::genai::utils::read_json_param(config, "thinker_config.classify_num", classify_num);
    OPENVINO_ASSERT(classify_num == QWEN3_FORCED_ALIGNER_CLASSIFY_NUM,
                    "Model at '", models_path.string(), "' is not a Qwen3 forced aligner: thinker_config.classify_num=",
                    classify_num, " (expected ", QWEN3_FORCED_ALIGNER_CLASSIFY_NUM, ").");

    m_timestamp_token_id = config.at("timestamp_token_id").get<int64_t>();
    m_timestamp_segment_ms = config.at("timestamp_segment_time").get<float>();
...
std::vector<ASRDecodedResultChunk> Qwen3ForcedAligner::align(const std::vector<float>& chunk_wav,
                                                             const std::string& text,
                                                             const std::string& language,
                                                             float chunk_offset_sec) {
    const std::string canonical_language = resolve_language(language);

    const std::vector<std::string> units = segment_alignment_units(text, canonical_language);
    if (units.empty()) {
        return {};
    }

    const WhisperFeatures features = m_feature_extractor.extract(chunk_wav, false);
    ...
    for (size_t unit_index = 0; unit_index < units.size(); ++unit_index) {
        const float start_ts = static_cast<float>(fixed_ms[unit_index * 2]) / 1000.0f + chunk_offset_sec;
        const float end_ts = static_cast<float>(fixed_ms[unit_index * 2 + 1]) / 1000.0f + chunk_offset_sec;
